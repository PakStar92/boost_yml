...
exports.buildWatermarkFilter = buildWatermarkFilter;
exports.buildTextWatermarkFilter = buildTextWatermarkFilter;
const FFmpeg_js_1 = require("../FFmpeg.js");
const binary_js_1 = require("../utils/binary.js");
function positionToOverlay(pos, margin) {
    switch (pos) {
...
async function addWatermark(opts) {
    const { input, watermark, output, position = 'bottom-right', margin = 10, opacity = 1, scaleWidth, videoCodec = 'libx264', audioCodec = 'copy', binary = (0, binary_js_1.resolveBinary)(), } = opts;
    ...
    const transforms = [];
    if (scaleWidth)
        transforms.push(`scale=${scaleWidth}:-1`);
    if (opacity < 1)
        transforms.push('format=rgba', `colorchannelmixer=aa=${opacity}`);
    // Always need at least one filter between [1:v] and [wm]
    if (transforms.length === 0)
        transforms.push('copy');
    const wmarkFilter = `[1:v]${transforms.join(',')}[wm]`;
    const filterComplex = `${wmarkFilter};[0:v][wm]overlay=${overlayExpr}[out]`;
    await new FFmpeg_js_1.FFmpegBuilder([input, watermark])
        .output(output)
...
async function addTextWatermark(opts) {
    const { input, output, text, position = 'bottom-right', margin = 10, fontSize = 24, fontColor = 'white@0.8', fontFile, videoCodec = 'libx264', audioCodec = 'copy', binary = (0, binary_js_1.resolveBinary)(), } = opts;
    let x, y;
    switch (position) {
        case 'top-left':
            x = `${margin}`;
...
// ─── Arg builders ─────────────────────────────────────────────────────────────
function buildWatermarkFilter(position, margin, opacity, scaleWidth) {
    const overlayExpr = positionToOverlay(position, margin);
    const transforms = [];
    if (scaleWidth)
        transforms.push(`scale=${scaleWidth}:-1`);
...
function buildTextWatermarkFilter(text, position, margin, fontSize, fontColor, fontFile) {
    let x, y;
    switch (position) {
        case 'top-left':
            x = `${margin}`;
