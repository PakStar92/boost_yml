...
exports.AudioFilterChain = exports.VideoFilterChain = exports.FilterGraph = exports.MultiInputNode = exports.GraphNode = exports.GraphStream = void 0;
exports.resetLabelCounter = resetLabelCounter;
exports.videoFilterChain = videoFilterChain;
exports.audioFilterChain = audioFilterChain;
exports.filterGraph = filterGraph;
const filters_js_1 = require("../types/filters.js");
let _labelCounter = 0;
function autoLabel(prefix = 'p') {
...
class GraphNode {
    ...
    colorkey(color, similarity = 0.1, blend = 0.0) {
    ...
    _commit(outputs) {
        ...
        const filterStr = this.chain.map(filters_js_1.serializeNode).join(',');
...
class MultiInputNode {
    graph;
    streams;
    constructor(graph, streams) {
        this.graph = graph;
        this.streams = streams;
    }
    overlay(x = 0, y = 0, opts = {}) {
        const named = { ...opts };
    ...
    hstack(inputs, shortest) {
        const named = {};
        if (inputs !== undefined)
            named['inputs'] = inputs;
        if (shortest !== undefined)
            named['shortest'] = shortest ? 1 : 0;
        return new GraphNode(this.graph, this.streams).filter('hstack', [], named);
    }
    vstack(inputs, shortest) {
        const named = {};
        if (inputs !== undefined)
            named['inputs'] = inputs;
        if (shortest !== undefined)
...
exports.MultiInputNode = MultiInputNode;
/**
...
class FilterGraph {
     ...
     */
    from(...refs) {
        const streams = refs.map((r) => typeof r === 'string'
...
class VideoFilterChain {
    chain = new filters_js_1.FilterChain();
    raw(str) { this.chain.raw(str); return this; }
    ...
    vflip() { this.chain.add({ name: 'vflip', positional: [], named: {} }); return this; }
    hflip() { this.chain.add({ name: 'hflip', positional: [], named: {} }); return this; }
    format(fmt) {
        this.chain.add({ name: 'format', positional: [fmt], named: {} });
        return this;
    }
    unsharp(lx = 5, ly = 5, la = 1.0) {
    ...
    eq(brightness, contrast, saturation, gamma) {
        ...
        if (brightness !== undefined)
            named['brightness'] = brightness;
        if (contrast !== undefined)
            named['contrast'] = contrast;
        if (saturation !== undefined)
            named['saturation'] = saturation;
        if (gamma !== undefined)
            named['gamma'] = gamma;
    ...
    toString() { return this.chain.toString(); }
}
exports.VideoFilterChain = VideoFilterChain;
/**
