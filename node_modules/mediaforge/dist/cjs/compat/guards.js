...
exports.guardVersion = guardVersion;
exports.guardFeatureVersion = guardFeatureVersion;
exports.guardCodec = guardCodec;
exports.guardFilter = guardFilter;
exports.guardHwaccel = guardHwaccel;
exports.guardCodecFull = guardCodecFull;
exports.assertCodec = assertCodec;
exports.assertHwaccel = assertHwaccel;
exports.assertFeatureVersion = assertFeatureVersion;
exports.selectBestCodec = selectBestCodec;
exports.selectBestHwaccel = selectBestHwaccel;
 ...
 */
function guardVersion(version, minMajor, featureName, minMinor = 0) {
    if ((0, version_js_1.satisfiesVersion)(version, minMajor, minMinor)) {
 ...
 */
function guardFeatureVersion(version, featureKey) {
    const gate = features_js_1.FEATURE_GATES[featureKey];
    if (gate === undefined) {
        return { available: false, reason: `Unknown feature: "${featureKey}"` };
    }
    const ok = (0, features_js_1.isFeatureExpected)(featureKey, version.major, version.minor);
    if (ok)
        return { available: true };
    return {
        available: false,
 ...
 */
function guardCodec(registry, codec, direction = 'encode') {
 ...
 */
function guardFilter(registry, filterName) {
 ...
 */
function guardHwaccel(registry, accelName) {
 ...
 */
function guardCodecFull(version, registry, codec, direction = 'encode', featureKey) {
 ...
 */
function assertCodec(registry, codec, direction = 'encode') {
 ...
 */
function assertHwaccel(registry, accelName) {
 ...
 */
function assertFeatureVersion(version, featureKey) {
...
class GuardError extends Error {
    alternative;
    constructor(message, alternative) {
...
function getCodecAlternative(codec) {
...
function getFilterAlternative(name) {
    return FILTER_ALTERNATIVES[name];
}
const HWACCEL_ALTERNATIVES = {
...
function getHwaccelAlternative(name) {
 ...
 * ]);
 */
function selectBestCodec(version, registry, candidates) {
    let lastAvailableSoftwareFallback = null;
 ...
 */
function selectBestHwaccel(registry, candidates) {
    for (const accel of candidates) {
        if (guardHwaccel(registry, accel).available)
            return accel;
    }
    return null;
}
