...
import { probeVersion, satisfiesVersion, formatVersion, } from './utils/version.js';
import { buildGlobalArgs, buildInputArgs, buildOutputArgs, toBitrate, } from './utils/args.js';
import { spawnFFmpeg, runFFmpeg } from './process/spawn.js';
import { CapabilityRegistry } from './codecs/registry.js';
import { guardCodec, guardHwaccel, guardFeatureVersion, selectBestCodec, selectBestHwaccel, GuardError, } from './compat/guards.js';
export class VersionError extends Error {
    constructor(feature, required, actual) {
        super(`"${feature}" requires FFmpeg v${required}+, but found v${actual}. ` +
...
export class FFmpegBuilder {
     ...
     */
    async selectVideoCodec(candidates) {
        const v = await this.getVersion();
     ...
     */
    selectHwaccel(candidates) {
        return selectBestHwaccel(this.getRegistry(), candidates);
    }
    requireVersion(minMajor, feature) {
        if (this._version === null)
            return; // version not yet known — defer
    ...
    inputFormat(format) {
        const last = this._inputs[this._inputs.length - 1];
        if (last !== undefined)
            last.opts.format = format;
        return this;
    }
    // ─── Output ───────────────────────────────────────────────────────────────
    ensureOutput() {
        if (this._currentOutput === null) {
            throw new Error('No output defined. Call .output("path") before codec/filter options.');
        }
